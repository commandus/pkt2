...
syntax = "proto3";
package example1;
import "pkt2.proto";    // обязательно включить файл с описанием опций
/// Temperature
...
message IE_IOHeader
{
    ...
    option(pkt2.packet) = { 
        ...
        {
```
тег (1) и фиксированый размер тела (28 байт) могут считаться признаком того, что пакет с тегами 1 и 28 - заголовок IOHeader, и их следует отметить признаком tag со значениями 1 и 28 соответсвенно.

Так как поле ie_size двухбайтное, то для него указывается порядок байт 
...
В опции pkt2.variable.get указывается, что значение переменной берется из поля value (field.value) и множается на константу. 
