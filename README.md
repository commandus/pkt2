...
| sint32               | Более эффективен для отрицательных чисел |
| sint64               | Более эффективен для отрицательных чисел |
...
Во втором случае нужно халать опцию format. В примере:
```
        format: ["value.degrees_c.toFixed(2).replace('.', ',')"]
```
градусы отобоажаются с точностью до второго знака после запятой, и вместо разделителя дробнрой и целой частей (".")
вставляется знак ",".
