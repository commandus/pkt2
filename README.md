...
Программа pkt2dumppq читает пакеты из шины пакетов и записывает их в базу данных напрямую.


Программа pkt2dumpfcm читает пакеты из шины пакетов и отправляет на мобильные устройства через сервис FireBase Cloud Messaging неразобранными пакетами.

Нужно указать подключение к базе данных, треьования к таблицам базы даннх см. раздел про обработчик handlerfcm.

handlerfcm, в отличие от дампера, отравляет разобранный пакет как объект, сериализованный в JSON.
...
```
pkt2dumpfcm -x 01004e01001c83c9145933303032333430313031353731333000000800005a822ce403000b003e08f68195cb0000000202001e0890003e01cf12000031812b70160000450002208a0086d04d244d000a00
```
...
В отличие от этого, дампер pkt2dumpfcm также отправлет на зарегистрированные мобильные усройства пользователяв посредством FireBase Cloud Messaging, но неразобранный пакет в виде строки шестнадцатиричных чисел.
